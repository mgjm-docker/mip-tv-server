...
// api version (root path)
const VERSION = '/v1'

// handlers for GET requests
const getHandler = Object.create(null);
getHandler[VERSION + '/locations/daily'] = require('./get/locations');
getHandler[VERSION + '/stats/servers'] = require('./get/servers');
getHandler[VERSION + '/stats/countries'] = require('./get/countries');

// handlers for POST requests
const postHandler = Object.create(null);
postHandler[VERSION + '/upload'] = require('./upload');
	...
	switch(req.method) {
		case 'GET': {
			...
			if(handler !== undefined) {
				...
				// data.date = Date.now() - 1000 * 60 * 60 * 8;
				// ... and run the handler
